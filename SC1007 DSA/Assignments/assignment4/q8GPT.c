...
int HashDelete(HashTable* Q3HashPtr, int key)
{
    ...
    // Find the index for the key using the hash function
    int index = Hash(key, Q3HashPtr->hSize);

    // Start from the head of the linked list in the corresponding bucket
    ListNode *current = Q3HashPtr->Table[index].head;
    ListNode *previous = NULL;

    // Traverse the list to find the key
    while (current != NULL) {
        if (current->key == key) {
            // Key found, now delete the node
            if (previous == NULL) {
                // Case 1: The node to delete is the head
                Q3HashPtr->Table[index].head = current->next;
            } else {
                // Case 2: The node to delete is in the middle or end
