...
{
    int index = 0;
    while (cur != NULL) { // Note: This is how you traverse the original linked list
        if (index % 2 == 0) {
            insertNode(ptrEvenList, index / 2, cur->item); // even indices
        }
        else {
            insertNode(ptrOddList, index / 2, cur->item); // odd indices
        }
