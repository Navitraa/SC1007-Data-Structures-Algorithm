...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    // parse key through hash function to obtain the index
    index = Hash(key, Q3HashPtr->hSize);

    // create newNode containing the key
    newNode = (ListNode *) malloc(sizeof(ListNode));
    newNode->key = key;

    // insert the key (update links + update count)
    newNode->next = Q3HashPtr->Table[index].head;
    Q3HashPtr->Table[index].head = newNode;

    Q3HashPtr->nSize++;
    Q3HashPtr->Table[index].size++; // NOTE syntax !!!


    // return 1 if node has been inserted successfully 
    return 1;
}
