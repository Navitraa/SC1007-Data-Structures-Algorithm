...
int moveMaxToFront(ListNode **ptrHead)
{
    ...
    // Special case: linkedlist with only 1 node
    if ((*ptrHead)->next == NULL) {
        return 0;
    }

    // Get maxValue - set initial first
    cur = (*ptrHead);
    maxItem = (*ptrHead)->item;

    // Get maxValue - the process
    while (cur != NULL) {
        if (cur->item > maxItem) {
            maxItem = cur->item;
            preMax = pre;
    ...
    // Special case: maxValue is already present at head node
    if (preMax == NULL) {
        return 0;
    }

    // Move maxValue to the first position - remove node, then insert at the front
    cur = preMax->next;
