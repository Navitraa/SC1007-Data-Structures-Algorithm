...
{
   // Base case: note that base case shld focus on the current node
    if (node == NULL) {
        return 0;
    }
   ...
   // Recursive case
    if ((node->left == NULL && node->right != NULL) || (node->left != NULL && node->right == NULL)) {
        count++; // count one child node
    }

    if (node->left != NULL) { //recurse through left subtree
        countOneChildNodes(node->left);
    }
    if (node->right != NULL) { //recurse through right subtree
