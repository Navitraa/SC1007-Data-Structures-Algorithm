...
void adjL2adjM(Graph *g){
    ...
    // error handling: return input if graph is already in adjacency matrix form
    if (g->type == ADJ_MATRIX) {
        printf("Error: graph is already in adjacency matrix form \n");
        return;
    }
    ...
    // create empty adjacency matrix first -- NOTE logic! (pls explain)
    g->adj.matrix = (int **) malloc(g->V * sizeof(int*));
    for (i=0; i<g->V; i++) {
        g->adj.matrix[i] = (int *)malloc(g->V * sizeof(int));
    }

    // initialise matrix with '0's (no edges initially)
    for (i=0; i<g->V; i++) {
        for (j=0; j<g->V; j++) {
            g->adj.matrix[i][j] = 0;
        }
    ...
    // iterate over adjacency list to fill the adjacency matrix
    for (i=0; i<g->V; i++) {
        temp = g->adj.list[i];
        while (temp != NULL) {
            g->adj.matrix[i][temp->vertex - 1] = 1; // NOTE: set matrix[i][j-1] bc vertex numbers are 1-based (REVISE when to +1 or -1!)
            temp = temp->next;
        }
    }
    
    // update graph type to ADJ_MATRIX after conversion
    g->type = ADJ_MATRIX;

    // free memory for ADJ_LIST[i] since it is no longer needed
    for (i=0; i<g->V; i++) {
    ...
    // free entire ADJ_LIST
    free(g->adj.list);

    // update graph structure to point to the newly-created ADJ_MATRIX
}
